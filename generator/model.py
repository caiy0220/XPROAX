...
from generator.noise import noisy
...
class DAE(TextModel):
    ...
    def encode(self, inputs):
        inputs = self.drop(self.embed(inputs))
        _, (h, _) = self.E(inputs)
        h = torch.cat([h[-2], h[-1]], 1)
        return self.h2mu(h), self.h2logvar(h)

    def decode(self, z, inputs, hidden=None):
        inputs = self.drop(self.embed(inputs)) + self.z2emb(z)
    ...
    def generate(self, z, max_len, alg):
        ...
        sents = []
        inputs = torch.zeros(1, len(z), dtype=torch.long, device=z.device).fill_(self.vocab.go)
        hidden = None
        for l in range(max_len):
            sents.append(inputs)
            logits, hidden = self.decode(z, inputs, hidden)
            if alg == 'greedy':
                inputs = logits.argmax(dim=-1)
            elif alg == 'sample':
                inputs = torch.multinomial(logits.squeeze(dim=0).exp(), num_samples=1).t()
            elif alg == 'top5':
                not_top5_indices=logits.topk(logits.shape[-1]-5,dim=2,largest=False).indices
                logits_exp=logits.exp()
                logits_exp[:,:,not_top5_indices]=0.
                inputs = torch.multinomial(logits_exp.squeeze(dim=0), num_samples=1).t()
        return torch.cat(sents)

    def forward(self, inputs, is_train=False):
        _input = noisy(self.vocab, inputs, *self.args.noise) if is_train else inputs
